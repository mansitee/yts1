...
module.exports = async function Download(
  ...
  // Regex untuk mengambil nilai dari validasi URL
  ...
  try {
    ...
    youtube.thumbnail = $('.thumbnail.cover img').attr('src');
    ...
    if (!regexUniqId.test(response.result))
      throw new Error('Video tidak dapat di download');
    ...
    if (error.message === "Cannot read property '0' of null") {
      throw new Error(`Kualitas Video yang diminta tidak tersedia`);
    } else {
